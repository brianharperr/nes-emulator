...
impl Ppu2 {
    ...
    pub fn step(&mut self){
        ...
        println!("S:{} C:{}", self.scanline, cycle);
        (|| {
            match scanline {
            Scanline::Visible | Scanline::PreRender => {
                if cycle == 0 { return; }
                
                if scanline == Scanline::PreRender {
                    if cycle == 1 {
                        self.status &= !0xE0;
    ...
    fn skip_cycle(&mut self) {
        if self.cycle == 340 && self.odd_frame && self.is_bg_rendering_enabled() { self.scanline = 0; self.cycle = 0; }
    } 
        ...
        if self.is_bg_rendering_enabled() {
            match self.cycle % 8 {
                0 => {
                    ...
                    let attr_bits = (self.at_byte >> shift) & 0x3;
                ...
                7 => {
                    let addr = self.bg_pattern_table_address() + (self.nt_byte as u16 * 16) + self.fine_y();
                    self.pt_latch_lo = self.read(addr + 8);
                }
                _ => {}
            }
        }
